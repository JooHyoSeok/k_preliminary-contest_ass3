...
class AstarPlan():
    ...
    def state_is_free(self, state): return (self.occ_grid[state.x-1:state.x+1, state.y-1:state.y+1] == 0).all()
        # 주변 상태가 갈 수 있는지 판단하는 함수
    ...
    def _follow_parent_pointers(self, parents, state):
        '''
        시작 상태까지의 최단 경로를 구한다.
        return : list: 최단 경로의 상태를 역순으로 포함하는 목록(시작 상태에서 지정된 상태로).
         '''
